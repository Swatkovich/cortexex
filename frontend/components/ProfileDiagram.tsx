...
export default function ProfileDiagram({ counts }: { counts: { dontKnow: number; know: number; wellKnow: number; perfectlyKnow: number } }) {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4 text-center sm:flex-row sm:items-center sm:gap-6 sm:text-left">
      <div style={{ width: size, height: size, position: 'relative' }} className="relative flex-shrink-0">
        ...
        <div className="absolute left-1/2 top-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-dark/50 text-sm font-semibold text-light">
          {total === 0 ? '—' : `${Math.round(((perfectlyKnow) / Math.max(1, total)) * 100)}%`}
        </div>
      </div>

      <div className="w-full text-sm text-light/70">
