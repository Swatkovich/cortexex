import Link from 'next/link';
import { observer } from 'mobx-react-lite';
import { themeStore } from '@/store/themeStore';

const PlayPage = observer(() => {
  const hasSelection = themeStore.selectedThemes.length > 0;

  return (
    <main className="mx-auto flex min-h-[calc(100vh-4rem)] max-w-3xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-0">
      <header>
        <p className="text-sm font-semibold uppercase tracking-wide text-light">
          Play Mode
        </p>
        <h1 className="mt-2 text-3xl font-semibold text-light">
          {hasSelection ? 'Get ready!' : 'No themes selected'}
        </h1>
        <p className="mt-2 text-light">
          {hasSelection
            ? 'You can now start the interactive session with the selected themes.'
            : 'Head back and choose at least one theme to unlock play mode.'}
        </p>
      </header>

      {hasSelection ? (
        <section className="space-y-3 rounded-2xl border border-light bg-dark p-6 shadow-[0_0px_60px_rgba(249,249,223,0.25)]">
          {themeStore.selectedThemes.map((theme) => (
            <article key={theme.id} className="border-b border-light pb-4 last:border-none last:pb-0">
              <h2 className="text-xl font-semibold text-light">{theme.title}</h2>
              <p className="mt-1 text-sm text-light">{theme.description}</p>
              <p className="mt-2 text-xs font-medium text-light">
                Difficulty: {theme.difficulty} · {theme.questions} questions
              </p>
            </article>
          ))}
          <p className="text-sm text-light">
            This is a placeholder view. Hook it to the gameplay experience whenever you&apos;re ready.
          </p>
        </section>
      ) : (
        <section className="rounded-2xl border border-dashed border-light p-6 text-center text-sm text-light">
          Nothing to load yet — add themes first.
        </section>
      )}

      <div className="mt-auto">
        <Link
          href="/user"
          className="inline-flex items-center justify-center rounded-xl border border-light px-6 py-3 text-base font-semibold text-light transition hover:bg-light-hover hover:text-dark"
        >
          Back to Themes
        </Link>
      </div>
    </main>
  );
});

export default PlayPage;

