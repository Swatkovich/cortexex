...
export default function AuthPage() {
  ...
  return (
    <PageContainer fullHeight centered className="max-w-lg justify-center">
      <Card className="w-full p-10 backdrop-blur-sm">
          ...
          <div className="flex rounded-2xl border border-light/10 bg-dark/30 p-1">
            <button
              ...
              className={`flex-1 rounded-xl px-4 py-2.5 text-sm font-semibold transition ${
                !isRegister ? 'bg-light text-dark shadow-sm' : 'text-light/60 hover:text-light'
              }`}
            >
              {t('auth.tab.login')}
            </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <FormField label={t('auth.label.name')} htmlFor="name" required>
            <Input
              id="name"
              type="text"
              value={name}
          ...
          <FormField label={t('auth.label.password')} htmlFor="password" required>
            <Input
              id="password"
              type="password"
              value={password}
              ...
              placeholder={
                isRegister
                  ? t('auth.placeholder.password.create')
                  : t('auth.placeholder.password.enter')
              }
            />
          </FormField>

          {isRegister && (
            <FormField label={t('auth.label.confirmPassword')} htmlFor="confirmPassword" required>
              <Input
                id="confirmPassword"
                type="password"
          ...
          <Button type="submit" size="fluid" isLoading={formLoading}>
